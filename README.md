...
tch_bl_model <- mlp(
  ...
  # set_args(activation = "sigmoid") |>  # margin sempre para regressão
  set_mode("regression") |> 
  set_engine("brulee")
```
...
``` r
tch_bl_last_fit_model <- tch_bl_last_fit$.workflow[[1]]$fit$fit
# vip(tch_bl_last_fit_model,
#     aesthetics = list(color = "black", fill = "orange")) +
#     theme(axis.text.y=element_text(size=rel(1.5)),
#           axis.text.x=element_text(size=rel(1.5)),
#           axis.title.x=element_text(size=rel(1.5))
