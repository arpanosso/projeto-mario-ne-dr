...
## ÁRVORES DE DECISÃO - DECISION TREE - DT
...
```{r}
tch_dt_best_params <- select_best(tch_dt_tune_grid, metric = "rmse")
tch_dt_wf <- tch_dt_wf |> 
  finalize_workflow(tch_dt_best_params)
tch_dt_last_fit <- last_fit(tch_dt_wf, tch_initial_split)

## Criando os preditos
tch_test_preds <- bind_rows(
  collect_predictions(tch_dt_last_fit)  |> 
    mutate(modelo = "dt"))
...
```{r}
vip_plot <-  tch_dt_last_fit_model <-tch_dt_last_fit$.workflow[[1]]$fit$fit
...
```{r} 
tree_fit_rpart <- extract_fit_engine(tch_dt_last_fit) 
